...
public class VersaoCActivity extends Activity
{
	...
	{
		VersaoCActivity.selecionados = selecionados;
	}
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		if (VersaoCActivity.itens == null) {
			gerarItens();
		}

		TextView txvTotalItensLista = (TextView) findViewById(R.id.VersaoC_txvTotalItensLista);
		Spinner spnOrdenacao = (Spinner) findViewById(R.id.VersaoC_spnOrdenacao);
		ListView ltvItens = (ListView) findViewById(R.id.VersaoC_ltvItens);

		txvTotalItensLista.setText("Total de itens : " + String.valueOf(VersaoCActivity.selecionados.size()));
		ordenarListaItens();

		adapter = new VersaoCItemAdapter(
				this,
				R.layout.layout_versao_c_listview_itens,
				(ArrayList<Item>) VersaoCActivity.selecionados);
		...
		spnOrdenacao.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view, int pos, long id)
			{
				switch (pos) {
					case 1:
						VersaoCActivity.ordenacao = Ordenacao.NOME;
						break;
					case 2:
						VersaoCActivity.ordenacao = Ordenacao.DESCRICAO;
						break;
					default:
						VersaoCActivity.ordenacao = Ordenacao.ID;
						break;
				}
				ordenarListaItens();
		...
		int spnOrdenacaoPos;
		switch (VersaoCActivity.ordenacao) {
			case NOME:
				spnOrdenacaoPos = 1;
				break;
			case DESCRICAO:
	...
	{
		Intent intent = new Intent(this, VersaoCIncluirItensActivity.class);
		startActivity(intent);
	}

	private void ordenarListaItens()
	{
		Item.ordenarLista(VersaoCActivity.selecionados, VersaoCActivity.ordenacao);
	}

	private void gerarItens()
	{
		VersaoCActivity.itens = new ArrayList<Item>();
		VersaoCActivity.itens.add(new Item(
		...
		VersaoCActivity.selecionados = new ArrayList<Item>();
	}
}
